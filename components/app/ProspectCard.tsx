 ...
 * (or a "no website" placeholder), with the opportunity score over it. The
 * card links to the detail page; the Pursue action lives in a footer OUTSIDE
 * the link, so giving the order never navigates away from the deck. */
export default function ProspectCard({ prospect: p }: { prospect: Prospect }) {
  const score = Math.round(p.opportunity_score);

  return (
    <div className="group flex h-full flex-col overflow-hidden rounded-2xl bg-carbon-panel ring-1 ring-white/10 transition-all hover:-translate-y-0.5 hover:ring-white/25">
      <Link href={`/app/prospect/${p.id}`} className="flex flex-1 flex-col">
      ...
      {/* Pursue footer — outside the Link so giving the order never navigates */}
      <div className="border-t border-white/[0.06] p-3">
        <PursueButton prospectId={p.id} status={p.status} variant="card" />
      </div>
    </div>
  );
}
